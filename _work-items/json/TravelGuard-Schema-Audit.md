...
jsonld/
  ...
  travel-insurance/
    ...
    plans/
      index.json                         ← P1, /travel-insurance/plans (ItemList of the 7 cards on the live listing)
      ...
      pack-n-go.json                     ← P2, Pack N’ Go same-day plan (discovered via Quarantine Bundle link)
      annual.json                        ← P2, Annual multi-trip plan (live page; sitemap wrongly implied retired — built Jun 11)
    optional-coverage/
      index.json                         ← P1, ItemList of the 11 upgrades/bundles
      cancel-for-any-reason.json         ← P0 (new), CFAR page
      {10 more bundle/upgrade pages}.json ← P2, all built Jun 10 (Service + on-page FAQPage: baggage, medical, name-your-family, pet, pre-existing waiver, quarantine, rental-vehicle-damage, security, travel-inconvenience, wedding)
    trip-types/
  ...
  traveler-types/
    ...
    {10 more traveler types}.json        ← P2, all built Jun 10 (WebPage + Service + OfferCatalog + on-page FAQPage)
  travel-resources/
    travel-safety/
...
## 11. Follow-ups for the web/site team

Items found during the schema build-out (Jun 9–11, 2026) that are site-side fixes, not schema work. None block deployment of the JSON-LD files.

| # | Item | Detail | Schema impact |
|---|------|--------|---------------|
| 1 | **Deploy the per-page JSON-LD files** | Everything in `jsonld/` except `_sitewide.json` (already live) and the minimal FAQPage on /help-center/faqs is pre-deployment. Never deploy `_templates/`. | The whole point of the project |
...
| 7 | *(FYI, not a bug)* `claims.travelguard.com/status` 404s to curl | It's a JS-app route linked from the live claims page — fine for users, just fails naive URL checks. | None — kept in `help-center/claims.json` |
